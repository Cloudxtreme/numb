...
void trak_build_index(struct trak_t* trak)
{
...

  {
    ...
    {
      trak->chunks_[i].pos_ = stco_get_offset((unsigned char *)stco, i);
    }
  }

  {
    ...
    unsigned int i = stsc_get_entries((unsigned char *)stsc);
    while(i > 0)
    {
      ...
      stsc_get_table((unsigned char *)stsc, i, &stsc_table);
...

  {
    ...
    unsigned int sample_size = stsz_get_sample_size((unsigned char *)stsz);
    unsigned int s = 0;
    {
      unsigned int j;
    ...
    if(sample_size == 0)
    {
      trak->samples_size_ = stsz_get_entries((unsigned char *)stsz);
    }
    else
    {
    ...
    if(sample_size == 0)
    {
      ...
      for(i = 0; i != trak->samples_size_ ; ++i)
        trak->samples_[i].size_ = stsz_get_size((unsigned char *)stsz, i);
    }
    else
    {
      unsigned int i;
...

  {
    ...
    unsigned int s = 0;
    unsigned int entries = stts_get_entries((unsigned char *)stts);
    unsigned int j;
    for(j = 0; j < entries; j++)
    {
      unsigned int i;
      ...
      unsigned int sample_count;
      unsigned int sample_duration;
      stts_get_sample_count_and_duration((unsigned char *)stts, j,
                                         &sample_count, &sample_duration);
...

  {
    ...
    {
      unsigned int s = 0;
      unsigned int entries = ctts_get_entries((unsigned char *)ctts);
      unsigned int j;
      for(j = 0; j < entries; j++)
      {
        unsigned int i;
        unsigned int sample_count;
        unsigned int sample_offset;
        ctts_get_sample_count_and_offset((unsigned char *)ctts, j, &sample_count, &sample_offset);
        for(i = 0; i < sample_count; i++)
        {
